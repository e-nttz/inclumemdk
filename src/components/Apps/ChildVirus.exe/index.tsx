...
const ChildVirus: AppProps<ChildVirusProps> = () => {
  ...
  const activateAntivirus = () => {
    ...
    setBeforeInstall(false); // Met à jour `beforeInstall` lors de l'activation
  };
  ...
  return (
    <>
      {beforeInstall && !antivirusInstalled && (
        <div className="w-full h-full flex items-center justify-center absolute bg-black bg-opacity-50">
          <div className="bg-[#CACACA] rounded-xl w-[500px]">
            <div className="p-6">
              <p>Contrôle de compte utilisateur</p>
              <p className="text-xl">Voulez-vous autoriser cette application à
              apporter des modifications à votre appareil ?</p>
            </div>
            <div className="bg-white p-6">
              <div className="flex items-center mb-8 mt-4">
                <img src={ChildVirusLogo} alt="Logo" className="w-10 mr-4"/>
                <p className="text-lg font-semibold">ChildVirus</p>
              </div>
              <p>Editeur vérifié : Child virus.</p>
              <p className="mb-8">Origine du fichier : Disque dur sur cet ordinateur</p>
            </div>
            <div className="flex justify-between p-6">
              <button className="bg-[#0051A3] text-white w-[48%] py-2 rounded-xl" onClick={
                () =>{
                  setBeforeInstall(false);
                }
              }>Oui</button>
              <button className="bg-[#E8E8E8] text-black w-[48%] py-2 rounded-xl">Non</button>
            </div>
          </div>
        </div>
      )}
